...
- 透明柴犬头像；固定币对后，Chrome 原生角标以浅粉色背景和深色大字显示 4 个字符，支持价格或 24 小时涨跌幅和 5 / 10 / 15 秒轮换。取消固定后不显示角标。
- 现货与 USDT 永续分市场搜索、详情、加入 / 移除自选、固定到角标，准确展示市场和计价单位。
- 弹窗支持浅色 / 深色 / 跟随系统；工具栏价格使用统一配色，不随涨跌改变，也不再提供涨跌颜色切换。
- WebSocket 推送，REST 批量补数；断线重连、限流退避、请求超时、旧响应防覆盖。
- 断网保留最后价格；超过 60 秒显示缓存状态。工具栏保持统一配色，悬浮说明包含连接状态和更新时间。

固定后按钮显示「已固定 · 点击取消」，再次点击即可清空角标；也可在设置中点击「隐藏角标」。取消固定或移除已固定币对会停止轮播，不会自动固定其他币对；自选列表行情照常更新。无固定币对时图标不显示数字。已有版本保存的有效固定选择会保留，取消状态在刷新、重启后也会保留。

启用角标后固定显示 4 个字符：小数补末尾零（`.600`、`1.20`），整数不足时前置补零（`076k` 表示约 76,000），不改变数量级；`1e-5` 表示约 0.00001。无报价显示 `----`，连接中显示 `....`。使用 Chrome 原生角标以保持大号数字清晰可读；角标位于图标右下角，可能遮住柴犬头像的一部分。不同系统的角标像素宽度和字号由 Chrome 决定。极端数值显示 `TINY` / `HUGE`，完整价格以悬浮说明和详情为准。涨跌幅角标省略 `%`，绝对值达到 100% 时显示 `+99+` / `-99+`。USDT 是实际计价资产，界面不会把它替换成美元符号。轮换时当前币对以图标悬浮说明为准。
