...
2026-09-20 验证：可取消固定修复与独立外观分支叠加后，`npm run check` 通过 82 项单元 / 组件测试、类型检查、生产构建及资源检查；`npm run test:e2e` 通过 4 项隔离 Chromium 扩展测试。真实 Chromium 扩展测试通过 `chrome.action.getBadgeText()` 确认 BTC 的模拟报价 `0.6073` 显示为 `.607`，并检查背景色 `#fbe4e6`、文字色 `#30282b`。取消固定后原生角标清空；旧版颜色设置不再生效。

验收反馈指出分区绘制把数字缩得太小，因此恢复 Chrome 原生角标的字号和位置，同时统一浅粉底与深色字。浏览器负责原生角标的位置和字号，可能覆盖柴犬头像的一部分。断线状态仍可在悬停说明和弹窗中查看。此次仅构建 dist，未生成 ZIP。
